...
// =================================================================
// Obyek `select` untuk konsistensi query
// =================================================================
	...
	try {
		...
		const [
			...
			prisma.article.findFirst({
				...
				select: popularAndLatestArticleSelect,
			}),
		...
		res
			.status(500)
			.json({ error: "Gagal mengambil data untuk halaman utama." });
	}
};
...
export const searchSite = async (req, res) => {
	...
	if (!query || String(query).trim().length < 3)
		return res.json({ articles: [], plants: [], events: [] });
...
export const getArticles = async (req, res) => {
	// 1. Ambil parameter 'query' dan 'sortBy' dari request.
	const {
		page = 1,
		limit = 10,
		query,
	...
	// 2. Bangun klausa 'where' untuk filter dan PENCARIAN
	...
	// 3. Buat objek 'orderBy' secara dinamis berdasarkan 'sortBy'
	let orderBy = {};
	switch (sortBy) {
		case "popular":
			orderBy = { viewCount: "desc" };
	...
	try {
		const [totalArticles, articlesFromDb] = await prisma.$transaction([
			...
			prisma.article.findMany({
				...
				// 4. Gunakan objek 'orderBy' yang dinamis di sini
				orderBy,
			}),
		]);
...
export const getPlants = async (req, res) => {
	...
	if (search) {
		where.OR = [
			{ name: { path: ["id"], string_contains: search, mode: "insensitive" } },
	...
	try {
		...
		const processedPlants = plantsFromDb.map((plant) => {
			...
			const truncatedDescId =
				originalDescId.length > 150
					? originalDescId.substring(0, 150) + "..."
					: originalDescId;
			const truncatedDescEn =
				originalDescEn.length > 150
					? originalDescEn.substring(0, 150) + "..."
					: originalDescEn;
			return {
				...plant,
				description: { id: truncatedDescId, en: truncatedDescEn },
			};
		});
...
export const getEvents = async (req, res) => {
	...
	if (search) {
		where.OR = [
			{ title: { path: ["id"], string_contains: search, mode: "insensitive" } },
...
export const getArticleById = async (req, res) => {
	...
	try {
		...
		res
			.status(404)
			.json({ error: "Article not found or could not be updated." });
	}
};
...
export const getAboutPageData = async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ error: "Gagal mengambil data halaman Tentang Kami." });
	}
};
