...
import { Plus, Trash2, X, PlusCircle, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                ...
                <button onClick={() => openEditModal()} className="w-full sm:w-auto bg-lime-400 text-gray-900 font-bold py-2 px-4 rounded-lg hover:bg-lime-500 flex items-center justify-center gap-2 transition-colors"><Plus size={20} /> Tambah Tanaman</button>
            </div>
            ...
            {isEditModalOpen && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                    <div className="bg-[#003938] border-2 border-lime-400/50 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                        <div className="p-4 flex justify-between items-center border-b border-lime-400/30"><h3 className="text-xl sm:text-2xl font-bold text-gray-200">{editingPlant ? 'Ubah Data Tanaman' : 'Tambah Tanaman Baru'}</h3><button onClick={closeEditModal} className="text-gray-400 hover:text-white"><X size={24} /></button></div>
                        <div className="p-4 sm:p-6 space-y-4 overflow-y-auto">
                            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                                ...
                                <input type="text" name="scientificName" value={formData.scientificName || ''} onChange={handleInputChange} placeholder="Nama Ilmiah" className="md:col-span-2 w-full px-4 py-2 bg-transparent border border-lime-400/60 rounded-lg text-gray-200" />
                                <div className='md:col-span-2'><textarea value={formData.description?.id || ''} onChange={(e) => handleJsonChange('description', 'id', e.target.value)} placeholder="Deskripsi (Indonesia)" className="w-full px-4 py-2 bg-transparent border border-lime-400/60 rounded-lg text-gray-200" rows={3}></textarea></div>
                                <div className='md:col-span-2'><textarea value={formData.description?.en || ''} onChange={(e) => handleJsonChange('description', 'en', e.target.value)} placeholder="Deskripsi (Bahasa Inggris)" className="w-full px-4 py-2 bg-transparent border border-lime-400/60 rounded-lg text-gray-200" rows={3}></textarea></div>
                                <select name="plantTypeId" value={formData.plantTypeId || 0} onChange={handleInputChange} className="md:col-span-2 w-full px-4 py-2 bg-transparent border border-lime-400/60 rounded-lg text-gray-200"><option value={0} disabled>Pilih Tipe Tanaman</option>{plantTypes.map(pt => <option key={pt.id} value={pt.id} className="bg-[#003938] text-white">{pt.name[lang]}</option>)}</select>
                                ...
                                <div className="md:col-span-2"><h4 className="text-lg font-semibold text-lime-300 border-b border-lime-400/30 pb-2 mb-3">Toko Penjual</h4>{formData.stores?.map((store: { name: string, url: string }, index: number) => (<div key={index} className="flex flex-col sm:flex-row items-center gap-2 mb-2"><input type="text" value={store.name || ''} onChange={(e) => handleStoreChange(index, 'name', e.target.value)} placeholder="Nama Toko" className="w-full sm:w-1/2 px-4 py-2 bg-transparent border border-lime-400/60 rounded-lg text-gray-200" /><input type="url" value={store.url || ''} onChange={(e) => handleStoreChange(index, 'url', e.target.value)} placeholder="URL Toko" className="w-full sm:w-1/2 px-4 py-2 bg-transparent border border-lime-400/60 rounded-lg text-gray-200" /><button onClick={() => removeStoreField(index)} className="text-red-500 hover:text-red-400 p-1 flex-shrink-0"><Trash2 size={18}/></button></div>))}<button type="button" onClick={addStoreField} className="mt-2 flex items-center gap-2 text-sm font-semibold text-lime-300 hover:text-lime-200"><PlusCircle size={16}/> Tambah Toko</button></div>
                            </div>
                        </div>
                        <div className="p-4 flex flex-col sm:flex-row justify-end gap-3 border-t border-lime-400/30"><button onClick={closeEditModal} className="w-full sm:w-auto bg-gray-700 text-gray-200 font-bold py-2 px-4 rounded-lg hover:bg-gray-600">Batal</button><button onClick={handleSave} disabled={isMutating} className="w-full sm:w-auto bg-lime-400 text-gray-900 font-bold py-2 px-4 rounded-lg hover:bg-lime-500 disabled:bg-gray-500 disabled:cursor-wait">{isMutating ? 'Menyimpan...' : 'Simpan'}</button></div>
                    </div>
                </div>
            )}
            ...
            {isDetailModalOpen && viewingPlant && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                    <div className="bg-[#003938] border-2 border-lime-400/50 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                        <div className="p-4 flex justify-between items-center border-b border-lime-400/30"><h3 className="text-xl sm:text-2xl font-bold text-gray-200">{viewingPlant.name[lang]}</h3><button onClick={closeDetailModal} className="text-gray-400 hover:text-white"><X size={24} /></button></div>
                        <div className="p-4 sm:p-6 space-y-3 overflow-y-auto">
                            ...
                            {viewingPlant.plantType && <div><strong className="text-lime-400">Tipe Tanaman:</strong> <span className="text-gray-300">{viewingPlant.plantType.name[lang]}</span></div>}
                            ...
                            {viewingPlant.description.en && <div className="pt-2"><strong className="text-lime-400 block mb-2">Deskripsi (EN):</strong><p className="text-gray-300 whitespace-pre-wrap">{viewingPlant.description.en}</p></div>}
                            {viewingPlant.stores && viewingPlant.stores.length > 0 && (
                                <div className="pt-2"><strong className="text-lime-400 block mb-2">Toko Penjual:</strong><ul className="list-disc list-inside text-gray-300 space-y-1">{viewingPlant.stores.map((store, index) => (<li key={index}><a href={store.url} target="_blank" rel="noopener noreferrer" className="hover:text-lime-300 underline">{store.name}</a></li>))}</ul></div>
                            )}
                        </div>
                        <div className="p-4 flex flex-col sm:flex-row justify-end border-t border-lime-400/30"><button onClick={closeDetailModal} className="w-full sm:w-auto bg-gray-700 text-gray-200 font-bold py-2 px-4 rounded-lg hover:bg-gray-600">Tutup</button></div>
                    </div>
                </div>
            )}
        </div>
