...
// --- PERBAIKAN: Impor tipe disederhanakan ---
import { useLayoutData, type Category } from '../hooks/useLayoutData';
...
// =================================================================
// --- SUB-KOMPONEN (Tidak ada perubahan) ---
// =================================================================
    ...
    if (isLoading) {
        ...
        return (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
                {[...Array(8)].map((_, i) => (
                    <div key={i} className="border-2 border-lime-400/50 bg-[#004A49]/60 rounded-lg shadow-sm overflow-hidden animate-pulse">
    ...
    return (
      <>
        ...
        <div className="flex justify-center items-center gap-4 mt-12">
          <button onClick={() => setPage(old => Math.max(old - 1, 1))} disabled={page === 1 || isFetching} className="px-4 py-2 bg-lime-400 text-gray-900 font-bold rounded-lg disabled:bg-gray-500 disabled:cursor-not-allowed transition-colors">{t('previous_button')}</button>
          <span className="text-white font-medium">{pagination ? t('page_info').replace('{currentPage}', String(pagination.currentPage)).replace('{totalPages}', String(pagination.totalPages)) : ''}</span>
          <button onClick={() => setPage(old => (pagination && old < pagination.totalPages ? old + 1 : old))} disabled={!pagination || page === pagination.totalPages || isFetching} className="px-4 py-2 bg-lime-400 text-gray-900 font-bold rounded-lg disabled:bg-gray-500 disabled:cursor-not-allowed transition-colors">{t('next_button')}</button>
        </div>
     ...
     <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 2xl:px-60 py-12 sm:py-16">
        ...
        <div className="mb-12 p-3 bg-[#004A49]/60 border-2 border-lime-400/50 rounded-lg">
          <CustomDropdown 
            ...
            options={(layoutData?.plantTypes as Category[])?.map(pt => ({ value: pt.id, label: pt.name[currentLang] })) || []}
            icon={<Sprout size={16} />} 
          />
        </div>
