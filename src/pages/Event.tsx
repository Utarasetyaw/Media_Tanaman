...
import { Calendar, MapPin } from "lucide-react"; // ExternalLink tidak lagi dipakai di sini
...
const EventCard: FC<{ event: Event; isPast?: boolean; lang: "id" | "en" }> = ({
	event,
	isPast,
	lang,
}) => {
	const t = (key: keyof typeof eventsTranslations.id) =>
		eventsTranslations[lang]?.[key] || key;
	...
	const cardContent = (
		<div
			className={`bg-[#004A49]/60 border-2 border-lime-400/50 rounded-lg shadow-lg overflow-hidden h-full flex flex-col transition-all duration-300 group-hover:shadow-lime-400/20 group-hover:-translate-y-1 ${
				isPast ? "opacity-70" : ""
			}`}
		>
			...
			{/* PERBAIKAN: Struktur flexbox dioptimalkan & deskripsi ditambahkan */}
			<div className="p-4 flex flex-col flex-grow">
				<div className="flex-grow">
					<p className="text-sm text-lime-400 font-semibold">{displayDate}</p>
					<h4 className="text-lg font-bold text-gray-100 mt-1 group-hover:text-lime-300 line-clamp-2">
						{event.title[lang]}
					</h4>
				...
				<p className="text-sm text-gray-300 mt-4 flex items-center">
					<MapPin
						size={16}
	...
	if (event.eventType === "EXTERNAL" && event.externalUrl) {
		return (
			<a
...
const EventPage: FC = () => {
	...
	const t = (key: keyof typeof eventsTranslations.id) =>
		eventsTranslations[currentLang]?.[key] || key;
	...
	if (isLoading)
		return (
			<div className="bg-[#003938] min-h-screen text-center py-16 text-white">
				{t("loading")}
			</div>
		);
	if (isError)
		return (
			<div className="bg-[#003938] min-h-screen text-center py-16 text-red-400">
				{t("error")}
			</div>
		);
				...
				<div className="w-full mx-auto px-4 sm:px-6 lg:px-8 2xl:px-60 py-12 sm:py-16">
					...
					pastEvents.length === 0 ? (
						...
						<div className="space-y-16 sm:space-y-24">
										...
										<div className="w-full md:w-1/2 p-6 sm:p-8 flex flex-col justify-center">
											<div>
												...
												<h3 className="font-serif text-3xl font-bold text-white mb-3">
													{featuredEvent.title[currentLang]}
												</h3>
												...
												<Link
													...
													className="inline-block bg-lime-300 text-lime-900 font-bold py-3 px-6 rounded-lg hover:bg-lime-400 transition-colors duration-300"
												>
													{t("register_button")}
												</Link>
											</div>
							...
							{otherUpcomingEvents.length > 0 && (
								<section>
									<h3 className="font-serif text-3xl font-bold text-center text-lime-400 mb-8">
										{t("other_events")}
									</h3>
